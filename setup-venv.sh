...
if ! [ -f setup.py -a -f squarespace.py ]; then
    echo 'You must run this in the top level of squarespace!'
    exit 1
elif [ -d .squarespace-$PYTHON_VERSION -a "$1" != "-f" ]; then
...
if [ -d .squarespace-$PYTHON_VERSION ]; then
    while true; do
        ...
        case $answer in
            yes|YES)
                rm -rf .squarespace-$PYTHON_VERSION activate-$PYTHON_VERSION
                break
                ;;
            no|NO)
...
virtualenv -p python$PYTHON_VERSION .squarespace-$PYTHON_VERSION || exit
ln -s .squarespace-$PYTHON_VERSION/bin/activate activate-$PYTHON_VERSION || exit
